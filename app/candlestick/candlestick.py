...
def bullish_three_method_formation(candles_df,
                   ohlc=__default_ohlc,
                   is_reversed=False,
                   target=None):
    cndl = __create_object('BullishThreeMethodFormation', target)
    return cndl.has_pattern(candles_df, ohlc, is_reversed)

def bearish_three_method_formation(candles_df,
                   ohlc=__default_ohlc,
                   is_reversed=False,
                   target=None):
    cndl = __create_object('BearishThreeMethodFormation', target)
    return cndl.has_pattern(candles_df, ohlc, is_reversed)

def tweezer_tops(candles_df,
                   ohlc=__default_ohlc,
                   is_reversed=False,
                   target=None):
    cndl = __create_object('TweezerTops', target)
    return cndl.has_pattern(candles_df, ohlc, is_reversed)

def tweezer_bottoms(candles_df,
                   ohlc=__default_ohlc,
                   is_reversed=False,
                   target=None):
    cndl = __create_object('TweezerBottoms', target)
    return cndl.has_pattern(candles_df, ohlc, is_reversed)
